...
struct Ops {
	long long add = 0, mul = 0, divv = 0, cmp = 0;
	void reset() { add = mul = divv = cmp = 0; }
};
...
// == Нормы ==
double norm1_vec(const Vector& x) {
	double s = 0;
	for (double v : x) s += fabs(v);
	return s;
}
double norm1_mat(const Matrix& A) {
	...
	for (size_t j = 0; j < m; ++j) {
		double s = 0;
		for (size_t i = 0; i < n; ++i) s += fabs(A[i][j]);
		r = max(r, s);
	}
	return r;
}
...
struct Perm {
	...
	void apply_inverse(Vector& x) const {
		Vector y(x.size());
		for (size_t j = 0; j < x.size(); ++j) y[j] = x[p[j]];
		x.swap(y);
	}
};
...
FR forward_elimination(Matrix A, Vector b, const string& mode, Ops& op) {
	...
	for (size_t k = 0; k < n && !sing; ++k) {
		...
		if ((mode == "row" || mode == "full") && piv_j != k) {
			for (size_t i = 0; i < n; ++i)
				swap(A[i][piv_j], A[i][k]);
			cp.swap_pos(k, piv_j);
		}

		double piv = A[k][k];
		if (fabs(piv) <= numeric_limits<double>::epsilon()) {
			sing = true;
			break;
		}
...
Matrix invert_via_gauss(const Matrix& Ain, const string& mode, Ops& op, bool* ok = nullptr) {
	size_t n = Ain.size();
	Matrix A = Ain, Inv(n, vector<double>(n, 0.0));
		...
		if (mode == "partial") {
			...
			for (size_t i = k; i < n; ++i)
				for (size_t j = k; j < n; ++j) {
					op.cmp++;
					double v = fabs(A[i][j]);
					if (v > best) { best = v; piv_i = i; piv_j = j; }
				}
		}

		if (piv_i != k) {
			swap(A[piv_i], A[k]);
			swap(Inv[piv_i], Inv[k]);
		}
		if ((mode == "row" || mode == "full") && piv_j != k) {
			for (size_t i = 0; i < n; ++i) {
				swap(A[i][piv_j], A[i][k]);
				swap(Inv[i][piv_j], Inv[i][k]);
			}
			cp.swap_pos(k, piv_j);
		}
...
void run_and_show(const string& caption, const Matrix& A0, const Vector& b0, const string& pivot) {
	...
	if (fr.singular) {
		cout << "Система численно вырождена.\n";
		return;
	}
	...
	Ops opi;
	bool ok = false;
	Matrix Ainv = invert_via_gauss(A0, pivot, opi, &ok);
	if (ok) {
	...
	Vector r = matvec(A0, x, nullptr);
	for (size_t i = 0; i < r.size(); ++i) r[i] -= b0[i];
...
int main() {
	...
	// Пункт 2 — демонстрация плохой обусловленности матрицы Гильберта
	{
		cout << "\n================ Пункт 2. Плохо обусловленная матрица (Гильберта) ================\n";

		Matrix A = hilbert(N2);

		// Точное решение: x* = (1, ..., 1)^T
		Vector x_star(N2, 1.0);

		// Правая часть b = A * x*
		Ops tmp;
		Vector b = matvec(A, x_star, &tmp);

		cout << "\n---- Решение исходной системы Ax = b (b = A*1) ----\n";
		run_and_show("Матрица Гильберта: исходная система", A, b, PIVOT);

		// Малое возмущение правой части: b1 = b + c
		cout << "\n---- Возмущение правой части: b1 = b + c ----\n";

		Vector delta(N2);
		std::mt19937 gen(123);
		std::uniform_real_distribution<double> D(-1e-10, 1e-10);
		...
		cout << "||c||_1 = " << norm1_vec(delta) << "  (очень маленькая помеха в правой части)\n";

		run_and_show("Матрица Гильберта: возмущенная система", A, b_tilde, PIVOT);

		// Оценка ошибки решения x̃ - x*
		cout << "\n---- Ошибка решения после возмущения ----\n";

		// Ещё раз решаем, чтобы получить x̃ явно
		Matrix A_copy = A;
		Ops op2;
		auto fr = forward_elimination(A_copy, b_tilde, PIVOT, op2);
		if (!fr.singular) {
			Vector x_tilde = back_subst(fr.U, fr.b, op2);
			if (string(PIVOT) == "row" || string(PIVOT) == "full")
				fr.colperm.apply_inverse(x_tilde);

			Vector err(N2);
			for (size_t i = 0; i < N2; ++i)
				err[i] = x_tilde[i] - 1.0;

			cout << "||x1 - x*||_1 = " << norm1_vec(err) << "\n";
		}
		else {
			cout << "При решении возмущенной системы возникла численная вырожденность.\n";
		}

		cout << "\nКомментарий: матрица Гильберта имеет очень большое число обусловленности.\n"
			"Даже небольшое возмущение правой части c (по норме ~1e-10) приводит к\n"
			"существенному изменению решения x1, что и демонстрирует плохую обусловленность.\n";
	}
